...
(function () {
	 ...
	 */
	function initTabs() {
		var tabLinks  = document.querySelectorAll( '.lw-enable-tabs a' );
		var tabPanels = document.querySelectorAll( '.lw-enable-tab-panel' );
		...
		var form = document.querySelector( '.lw-enable-settings' );
		if (form) {
		...
		if (form) {
			form.addEventListener(
				'submit',
				function () {
					var activeLink = document.querySelector( '.lw-enable-tabs a.active' );
					if ( ! activeLink) {
						return;
					}
					var tabId    = activeLink.getAttribute( 'href' ).substring( 1 );
					var tabInput = form.querySelector( 'input[name="lw_enable_active_tab"]' );
					if (tabInput) {
